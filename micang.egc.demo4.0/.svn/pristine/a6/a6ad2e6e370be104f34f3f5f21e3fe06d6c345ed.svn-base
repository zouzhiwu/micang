...
public class KillHeroExpConfig {
	...
	public static void init() {
		InputStream in = KillHeroExpConfig.class.getClassLoader().getResourceAsStream("template/KillHeroExp.xlsx");
		try {
			...
			logger.info(String.format("kill hero Exp config loaded record %d", i - 2));
			workBook.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
