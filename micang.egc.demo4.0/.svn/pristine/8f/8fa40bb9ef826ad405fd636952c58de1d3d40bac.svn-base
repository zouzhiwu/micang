...
public class DuplicateAction {
	...
	public void chapterListInfo(Message message, Channel channel) throws Exception {
		...
		chapterList.stream().forEach(chapter -> {
			...
			List<PointsDaoBean> list = JSONObject.parseArray(chapter.getPoints(), PointsDaoBean.class);
			if (list != null) {
	...
	public void checkpointListInfo(Message message, Channel channel) throws Exception {
		...
		List<PointsDaoBean> list = JSONObject.parseArray(chapterInfo.getPoints(), PointsDaoBean.class);
		if (list == null) {
	...
	public void checkpointProcedures(Message message, Channel channel) throws Exception {
		Integer chapterId = message.getInt();
		Integer checkpointId = message.getInt();
		logger.info(String.format("RESV %d from accountId=%s chapterId=%d checkpointId=%d", message.getMsgcd(),
				message.getAccountId(), chapterId, checkpointId));
		System.out.println(String.format("RESV %d from accountId=%s chapterId=%d checkpointId=%d", message.getMsgcd(),
				message.getAccountId(), chapterId, checkpointId));
		duplicateService.checkpointProcedures(message.getAccountId(), chapterId, checkpointId);
	...
	@Action(MessageCode.msg_duplicate_get_saoDang)
	public void saoDang(Message message, Channel channel) throws Exception {
		Integer chapterId = message.getInt();
		Integer pointId = message.getInt();
		Byte type = message.getByte();
		logger.info(String.format("RESV %d from accountId=%s chapterId=%d pointId=%d type=%d ", message.getMsgcd(),
				message.getAccountId(), chapterId, pointId, type));
		Byte results = duplicateService.saoDang(message.getAccountId(), chapterId, pointId, type);
