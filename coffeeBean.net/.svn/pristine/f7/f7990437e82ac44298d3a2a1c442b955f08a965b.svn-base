...
public class BinaryWebSocketFrameHandler extends SimpleChannelInboundHandler<BinaryWebSocketFrame>{
    ...
    protected void channelRead0(ChannelHandlerContext ctx, BinaryWebSocketFrame msg) throws Exception {
    	...
    	if (length != buffer.capacity() - buffer.readerIndex()) {
    		ctx.close();
            return;
    	}
    	Message message = MsgDecoder.doDecode(ctx.channel(), length, buffer);
...
//				action.execute(message, ctx.channel());
				...
				logger.info(String.format("0x%04X from roleId=%d infomessage:%s", message.getMsgcd(), message.getAccountId(), exception.getClientInfo()));
