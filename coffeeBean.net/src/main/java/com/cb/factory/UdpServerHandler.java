...
public class UdpServerHandler extends SimpleChannelInboundHandler<DatagramPacket> {
	...
	protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket datagramPacket) throws Exception {
        ...
        if (message.getMsgcd() == 2001) {
            ...
            MsgSender.putAddress(message.getAccountId(), address);
        }
        MsgSender.putUdpChannel(message.getAccountId(), ctx.channel());
...
//		AbsAction<Message> action = (AbsAction<Message>)listener.getAction(msgcdType);
		...
		if (null == methodClazz) {
			...
			if (isRight) {
				boolean isHasChannel = MsgSender.hasAddress(message.getAccountId());
				if (isHasChannel) {
					try {
//						action.execute(message, ctx.channel());
						...
						logger.info(String.format("msgcd=%d from roleId=%d infomessage:%s", message.getMsgcd(), message.getAccountId(), exception.getClientInfo()));
	...
	public static Message doDecode(ByteBuf buffer) {
		...
		Long accountId = buffer.readLong();
