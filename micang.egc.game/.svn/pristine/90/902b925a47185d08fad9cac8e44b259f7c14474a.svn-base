		...
		try {
			...
			for (Iterator<Row> iterator = sheet.rowIterator(); iterator.hasNext();) {
				...
				if (i >= 2) {
					...
					int coin = PoiUtil.getInt((XSSFCell)row.getCell(index++));
					
					MemberDuanTemplate memberDuanTemplate = new MemberDuanTemplate(id, name, quality, ascending, material1, number, y1,
																v1,  y2, v2, y3, v3, y4, v4, y5, v5, grade,txt, coin);
