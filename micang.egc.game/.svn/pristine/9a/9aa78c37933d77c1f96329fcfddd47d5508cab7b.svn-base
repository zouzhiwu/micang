...
public class AStarService {
	 ...
	 * 判断结点能否放入Open列表
	 */
	private static boolean canAddNodeToOpen(MapInfo mapInfo, int x, int y, Map<Integer, Node> closeList) {
		// 如果不在地图中，则不能加入open列表
		if (x < 0 || x >= mapInfo.width || y < 0 || y >= mapInfo.hight) {
			return false;
		}
		// 如果结点是障碍，则不能加入open列表
		if (mapInfo.maps[y][x] == MapNodeType.Black.getIndex()) {
			return false;
		}
		// 如果结点在close表，则不能加入open列表
		if (isCoordInClose(mapInfo, x, y, closeList)) {
			return false;
		}
		return true;
	}
	 ...
	 * 判断坐标是否在close表中
	 */
	private static boolean isCoordInClose(MapInfo mapInfo, Coord coord, Map<Integer, Node> closeList) {
		int nodeKey = getNodeKey(coord.x, coord.y, mapInfo);
		return closeList.containsKey(nodeKey);
	}

	/**
	 * 判断坐标是否在close表中
	 */
	private static boolean isCoordInClose(MapInfo mapInfo, int x, int y, Map<Integer, Node> closeList) {
		if (closeList.isEmpty())
			return false;
		int nodeKey = getNodeKey(x, y, mapInfo);
		return closeList.containsKey(nodeKey);
	}
	 ...
	 * 6 添加所有邻结点到open列表
	 */
	private static void addNeighborNodeInOpen(MapInfo mapInfo, Node current, List<Node> openList, Map<Integer, Node> closeList) {
	 ...
	 * 添加一个邻结点到open表
	 */
	private static void addNeighborNodeInOpen(MapInfo mapInfo, Node current, int x, int y, int value, List<Node> openList, Map<Integer, Node> closeList) {
	...
	private static List<Node> drawPath(byte[][] maps, Node end) {
		...
		while (end != null) {
			Coord c = end.coord;
			maps[c.y][c.x] = MapNodeType.PATH.getIndex();
			end = end.parent;
			if (end != null) {
				nodeList.add(0, end);
			}
	...
	public static List<Node> searchWay(int startx, int starty, int endx, int endy, byte[][] maps) {
		...
		Map<Integer, Node> closeList = new HashMap<Integer, Node>();
		return searchWay(startx, starty, endx, endy, maps, openList, closeList);
	}

	/**
	 * (8) 开始算法，循环移动结点寻找路径，设定循环结束条件，Open表为空或者最终结点在Close表
	 */
	private static List<Node> searchWay(int startx, int starty, int endx, int endy, byte[][] maps, List<Node> openList, Map<Integer, Node> closeList) {
		openList.clear();
		closeList.clear();
		if (maps[starty][startx] == MapNodeType.Black.getIndex() || maps[endy][endx] == MapNodeType.Black.getIndex()) {
	 ...
	 * 移动当前结点
	 */
	private static List<Node> moveNodes(MapInfo mapInfo, List<Node> openList, Map<Integer, Node> closeList) {
		List<Node> nodeList = new ArrayList<Node>();
//		int num = 0;
		// 如果open列表不为空，则循环执行
		while (!openList.isEmpty()) {
			// 如果终点在close列表，说明已经找到了终点，则绘制路径
			if (isCoordInClose(mapInfo, mapInfo.end.coord, closeList)) {
				// 回溯法绘制路径
			...
			int nodeKey = getNodeKey(current.coord.x, current.coord.y, mapInfo);
			closeList.put(nodeKey, current);
...
//			logger.info(String.format("current(x=%d, y=%d)", current.coord.x, current.coord.y));
	...
	private static int getNodeKey(int x, int y, MapInfo mapInfo) {
		return x * mapInfo.hight + y;
	}
