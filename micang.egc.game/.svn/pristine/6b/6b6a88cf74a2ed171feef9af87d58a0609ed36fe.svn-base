...
public class RedisClient<T> {
	 ...
	 *
	  ...
	  * @return
	  */
	 public void setCacheMap(int roomId, Map<Long, T> dataMap){
		 if(dataMap.isEmpty()){
			 return;
		 }
	    HashOperations<String, String, T> hashOperations = redisTemplate.opsForHash();
	    Map<String, T> map = new HashMap<String, T>(dataMap.size());
	    for (Map.Entry<Long, T> entry : dataMap.entrySet()) {
	    	map.put(String.valueOf(entry.getKey()), entry.getValue());
	    }
	    hashOperations.putAll(String.valueOf(roomId), map);
	 }
	 
	 public void setCacheMap(int roomId, long roleId, T value) {
		 HashOperations<String, String, T> hashOperations = redisTemplate.opsForHash();
		 hashOperations.put(String.valueOf(roomId), String.valueOf(roleId), value);
	 }
	 
	 public T getCacheMap(int roomId, long roleId) {
		 HashOperations<String, String, T> hashOperations = redisTemplate.opsForHash();
		 return hashOperations.get(String.valueOf(roomId), String.valueOf(roleId));
	 }
	 
	 public Map<Long, T> getCacheMap(int roomId){
		  HashOperations<String, String, T> hashOperations = redisTemplate.opsForHash();
		  Map<String, T> dataMap = hashOperations.entries(String.valueOf(roomId));
		  Map<Long, T> map = new HashMap<Long, T>(dataMap.size());
