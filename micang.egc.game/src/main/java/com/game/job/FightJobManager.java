...
import com.game.job.fight.SyncFrameJob;
...
//	private final static Logger logger = LoggerFactory.getLogger(RoomJobManager.class);
	 ...
	 *  创建同步帧Job
	 */
	public static void createSyncFrameJob(Room room) {
		String jobName = UuidHelper.getUuid();
		String groupName = room.groupName;
		JobKey jobKey = new JobKey(jobName, groupName);
		Map<String, Object> dataMap = new HashMap<String, Object>();
		dataMap.put("room", room);
		long delayTime = 33;
		long repeatInterval = 33;
		JobScheduler.createRepeatJob(jobKey, delayTime, repeatInterval, dataMap, SyncFrameJob.class);
	}
