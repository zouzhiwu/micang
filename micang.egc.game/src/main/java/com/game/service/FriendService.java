...
public class FriendService {
	...
	public List<Friend> getFriendList(Long acountId) {
		...
		//查询我的好友
		List<FriendRelation> friends = friendDao.findFriendByRelation(acountId, (int)FriendRelationType.friends.getIndex());
		//封装返回对象
	...
	private Friend getFriendVO(Long acountId, FriendRelation friend) {
		...
		Integer bGivingA = (int) FriendRelationType.aBlackB.getIndex();
		Integer aGivingB = (int) FriendRelationType.bBlackA.getIndex();
		Integer eachGiving = (int) FriendRelationType.aApplyB.getIndex();
		//如
	...
	public Boolean setPhysicals(Long accountId, List<Long> friendsId) {
		//查询我的全部好友
		List<FriendRelation> friends = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.friends.getIndex());
		...
		friends.stream().forEach(friend -> {
			if (friend.getAccountId1().equals(accountId) && friendsId.indexOf(friend.getAccountId2()) != -1) {
				if (userRecord.getPutPhysical() > 0) {
					//1.我是a 他是b
					if(bGivingA == friend.getGiving()) {
						friend.setGiving((int)FriendRelationType.aBlackB.getIndex());
						//给我加体力
						addTIli(role);
				...
				if (userRecord.getPutPhysical() > 0) {
					//1.我是b 他是a
					if(aGivingB == friend.getGiving()) {
						friend.setGiving((int)FriendRelationType.bBlackA.getIndex());
						//给我加体力
						addTIli(role);
					} else if(eachGiving == friend.getGiving()){
						friend.setGiving(bGivingA);
...
//		Integer friends = (int) FriendRelationType.friends.getIndex();
//		Integer aApplyB = (int) FriendRelationType.aApplyB.getIndex();
//		Integer eachApply = (int) FriendRelationType.eachApply.getIndex();
//		Integer bApplyA = (int) FriendRelationType.bApplyA.getIndex();
//		Integer aShieldB = (int) FriendRelationType.aApplyB.getIndex();
//		Integer bShieldA = (int) FriendRelationType.aApplyB.getIndex();
...
//		return resVo;
	...
	public List<Friend> getFriendApply(Long accountId) {
		//查询加我的好友
		List<FriendRelation> friends1 = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.aApplyB.getIndex());
		List<FriendRelation> friends2 = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.bApplyA.getIndex());
		List<FriendRelation> friends3 = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.eachApply.getIndex());
	...
	private Integer addOrDel(Boolean isAgreed, Integer countFriend, Integer maxFriendNumber, FriendRelation friend) {
		if (isAgreed) {
			...
			friend.setRelation((int)FriendRelationType.friends.getIndex());
			friendDao.update(friend);
	...
	public List<Friend> getBlacklistFriend(Long accountId) {
		//查询加我的好友
		List<FriendRelation> friends1 = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.bBlackA.getIndex());
		List<FriendRelation> friends2 = friendDao.findFriendByRelation(accountId, (int)FriendRelationType.aApplyB.getIndex());
		List<Friend> friendVOs = new ArrayList<Friend>();
		friends1.stream().forEach(friend -> {
			//黑名单筛选，处理
	...
	public boolean addFrienToBlacklist(Long accountId, Long friendId) {
		...
		if (null != findById2) {
			findById2.setRelation((int)FriendRelationType.aBlackB.getIndex());
			friendDao.update(findById2);
		} else if (null != findById1) {
			findById1.setRelation((int)FriendRelationType.bBlackA.getIndex());
			friendDao.update(findById1);
		} else {
			FriendRelation friend = new FriendRelation();
			friend.setAccountId1(accountId);
			friend.setAccountId2(friendId);
			friend.setRelation((int)FriendRelationType.aApplyB.getIndex());
			friendDao.create(friend);
		}
		return true;
	}
