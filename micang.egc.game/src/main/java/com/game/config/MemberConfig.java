		...
		try {
			...
			for (Iterator<Row> iterator = sheet.rowIterator(); iterator.hasNext();) {
				...
				if (i >= 2) {
					...
					String unlockCotent = PoiUtil.getString((XSSFCell)row.getCell(index++));
					Short transformation = PoiUtil.getShort((XSSFCell)row.getCell(index++));
					
					MemberTemplate memberTemplate = new MemberTemplate(id, preMemberId, stars, name, itemId, smallIcon, bigIcon, image, awata,
							quality, work, dynamic, operation, focus, tough, potential,
							growthDynamic, growthOperation, growthFocus, growthTough,
							growthPotential, stageId, likeHeros, masterSkillId, slaveSkillIds, unlock,
							fragmentCount, characterGroupId, desc, unlockCotent, transformation);
